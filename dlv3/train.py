...
if __name__ == "__main__":
    ...
    if not (os.path.isfile('./img_list_csv/val.csv')
            or os.path.isfile('./img_list_csv/train.csv')
            or os.path.isfile('./img_list_csv/test.csv')
            ):
    ...
    #net =UNetWithResnet50Encoder(n_classes)
    net = deeplabv3p(n_classes)  #8个分类
    ...
    if Mode_Setting == '0':
        ...
        if (os.path.isfile('./deeplabv3p_baidulane.pth')):
                net.load_state_dict(torch.load('./deeplabv3p_baidulane.pth',map_location='cpu'))      
